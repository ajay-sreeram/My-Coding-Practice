...
System.out.println("Enter 1 for Encryption 2 fro Decryption");
...
public static void Encrypt()
{
	...
	System.out.println("Enter Text to Encrypt (Give String with even length) and should contain only alphabets:\n");
	...
	for(i=0;i<st.length();i+=2)
		result+=encryptFun(st.substring(i,i+2),key);
	System.out.println("Encryption text:"+result);
}
public static String encryptFun(String st,int keys[][])
{
	int p,q;
	p=(keys[0][0]*((int)st.charAt(0)-97)+keys[0][1]*((int)st.charAt(1)-97))%26+97;
	q=(keys[1][0]*((int)st.charAt(0)-97)+keys[1][1]*((int)st.charAt(1)-97))%26+97;
	return ""+(char)p+""+(char)q;
}



public static void Decrypt()throws java.lang.Throwable{
Scanner in=new Scanner(System.in);
System.out.println("Enter key(4 numbers) in single Line");
String temp[]=in.nextLine().split(" ");
int key[][];
