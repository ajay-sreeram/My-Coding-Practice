...
public class BigInt {
  int[] digits; // Stores the digits
  int size; // Stores the actual length of the BigInt in digits.
  public BigInt(int n) {
    // Special case of n=0.
    if (n == 0) {
      size = 1;
      ...
      // Peel off te digits starting with the least significant one, one
  ...
  // Adds othe to the current object and returns the answer.
  ...
  public int compare(BigInt other) {
    ...
    // All digts must have matched, the two values are equal.
    return 0;
  }
