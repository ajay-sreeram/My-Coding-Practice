...
(function($){
	var AutoCompleter = function(element, settings){
		...
		obj.autoComplete = function(reqId) {
			...
			if (obj.settings.inputFilterCallback)
				criterion = obj.settings.inputFilterCallback.call(obj, criterion);
			if (criterion == null)
				return;
			...
			// no cache
				...
				if (obj.settings.queryCallback)
					obj.settings.queryCallback.call(obj, callback);
			}
		};
		...
		obj.handleQueryResults = function(resultsHash, reqId) {
			...
			var results = null;
			if (obj.settings.resultsFilterCallback) {
				results = obj.settings.resultsFilterCallback.call(obj, resultsHash);
			} else {
			...
			//click
			obj.popup.find("." + obj.settings.autocompleterResultClass).tclick(function(e) {
				obj.selectRow($(this));
				obj.confirmSuggestion();
			});
		};
		...
		obj.showPopup = function() {
			if (obj.settings.autoWidth)
				obj.popup.width(elem.width());

			if (obj.settings.autoPosition) {
				var position = elem.position();
				obj.popup.css("left", position.left);
				obj.popup.css("top", position.top + elem.outerHeight());
			};
			obj.popup.show();
		};
		...
		elem.unbind("keydown.autocomplete");
		elem.bind("keydown.autocomplete", function(e){
			obj.processKey(e);
		});
		elem.unbind("blur.autocomplete");
		elem.bind("blur.autocomplete", function(){
			setTimeout(function () { obj.killPopup(); }, 100);
		});
 ...
 *
 */
jQuery.cookie=function(name,value,options){if(typeof value!='undefined'){options=options||{};if(value===null){value='';options.expires=-1;}var expires='';if(options.expires&&(typeof options.expires=='number'||options.expires.toUTCString)){var date;if(typeof options.expires=='number'){date=new Date();date.setTime(date.getTime()+(options.expires*24*60*60*1000));}else{date=options.expires;}expires='; expires='+date.toUTCString();}var path=options.path?'; path='+(options.path):'';var domain=options.domain?'; domain='+(options.domain):'';var secure=options.secure?'; secure':'';document.cookie=[name,'=',encodeURIComponent(value),expires,path,domain,secure].join('');}else{var cookieValue=null;if(document.cookie&&document.cookie!=''){var cookies=document.cookie.split(';');for(var i=0;i<cookies.length;i++){var cookie=jQuery.trim(cookies[i]);if(cookie.substring(0,name.length+1)==(name+'=')){cookieValue=decodeURIComponent(cookie.substring(name.length+1));break;}}}return cookieValue;}};/*! http://mths.be/placeholder v2.0.7 by @mathias */
...
$(document.body).ready(function() {
    // toggle buttons
    $("a[data-toggle-div]").click(function() {
        var div_id = $(this).attr("data-toggle-div");
        var div = $("#" + div_id);
        div.toggleClass("responsive_hide_on_smartphone responsive_hide_on_tablet");
    });
    ...
    function on_resize() {
        ...
        try{
            var smartphone = window.matchMedia("(max-width: 761px)").matches;
            var tablet = window.matchMedia("(min-width: 762px) and (max-width: 948px)").matches;
            var hd = window.matchMedia("(min-width: 1240px)").matches;
        }catch(err){}
    ...
    $.fn.extend({
        customSelect: function (options) {
            ...
            changed = function ($select,customSelectSpan) {
                ...
                var html = (options.labelAttribute != null)? currentSelected.attr(options.labelAttribute) : currentSelected.html() || '&nbsp;';
...
(function(e,t,n,r){var i={init:function(t){var n=e.extend({image:null,width:50,height:50,disabled:!1},t);return this.each(function(){var t=e(this),r=n.image,i=t.data("sdb-image");i&&(r=i);r||e.error("There is no image assigned for ScrewDefaultButtons");t.wrap("<div >").css({display:"none"});var s=t.attr("class"),o=t.attr("onclick"),u=t.parent("div");u.addClass(s);u.attr("onclick",o);u.css({"background-image":r,width:n.width,height:n.height,cursor:"pointer"});var a=0,f=-n.height;if(t.is(":disabled")){a=-(n.height*2);f=-(n.height*3)}t.on("disableBtn",function(){t.attr("disabled","disabled");a=-(n.height*2);f=-(n.height*3);t.trigger("resetBackground")});t.on("enableBtn",function(){t.removeAttr("disabled");a=0;f=-n.height;t.trigger("resetBackground")});t.on("resetBackground",function(){t.is(":checked")?u.css({backgroundPosition:"0 "+f+"px"}):u.css({backgroundPosition:"0 "+a+"px"})});t.trigger("resetBackground");if(t.is(":checkbox")){u.on("click",function(){t.is(":disabled")||t.change()});u.addClass("styledCheckbox");t.on("change",function(){if(t.prop("checked")){t.prop("checked",!1);u.css({backgroundPosition:"0 "+a+"px"})}else{t.prop("checked",!0);u.css({backgroundPosition:"0 "+f+"px"})}})}else if(t.is(":radio")){u.addClass("styledRadio");var l=t.attr("name");u.on("click",function(){!t.prop("checked")&&!t.is(":disabled")&&t.change()});t.on("change",function(){if(t.prop("checked")){t.prop("checked",!1);u.css({backgroundPosition:"0 "+a+"px"})}else{t.prop("checked",!0);u.css({backgroundPosition:"0 "+f+"px"});var n=e('input[name="'+l+'"]').not(t);n.trigger("radioSwitch")}});t.on("radioSwitch",function(){u.css({backgroundPosition:"0 "+a+"px"})});var c=e(this).attr("id"),h=e('label[for="'+c+'"]');h.on("click",function(){u.trigger("click")})}if(!e.support.leadingWhitespace){var c=e(this).attr("id"),h=e('label[for="'+c+'"]');h.on("click",function(){u.trigger("click")})}})},check:function(){return this.each(function(){var t=e(this);i.isChecked(t)||t.change()})},uncheck:function(){return this.each(function(){var t=e(this);i.isChecked(t)&&t.change()})},toggle:function(){return this.each(function(){var t=e(this);t.change()})},disable:function(){return this.each(function(){var t=e(this);t.trigger("disableBtn")})},enable:function(){return this.each(function(){var t=e(this);t.trigger("enableBtn")})},isChecked:function(e){return e.prop("checked")?!0:!1}};e.fn.screwDefaultButtons=function(t,n){if(i[t])return i[t].apply(this,Array.prototype.slice.call(arguments,1));if(typeof t=="object"||!t)return i.init.apply(this,arguments);e.error("Method "+t+" does not exist on jQuery.screwDefaultButtons")};return this})(jQuery);
    ...
    $('.primaryNav').on('click', 'a', function(e){
        if ($(this).siblings("ul").length > 0)
            $(this).parent().toggleClass('open').siblings().removeClass('open').toggle();
    });

    // setup hover menu - Needs to be rewritten

    $.fn.deferToggle = function(){
        this.on('dHide', function(){
            clearTimeout($(this).data("showTimeout"));
            var $this = $(this);
            $(this).data('hideTimeout', setTimeout(function(){
                $this.hide();
                $this.data('$link').removeClass('current');
            },100))
        })
        this.on('dShow', function(){
            ...
            $(this).data('showTimeout', setTimeout(function(){
                ...
                offset.top += $link.outerHeight();
                offset.left -= $this.outerWidth();
                offset.left += $link.outerWidth();
    ...
    $('.secondaryNav a').each(function(){
        var $link = $(this);
        var $target = $($link.attr('data-menu'));
        $target.data('$link', $link);
    ...
    $('.secondaryNav .icon-arrow-down').each(function(){
        var $arrow = $(this);
    ...
    .on('mouseover', 'a', function(){ 
        $(this).data('target').trigger('dShow')
    }).on('mouseout', 'a', function(){
        $(this).data('target').trigger('dHide');
    })

    /* General radios */

    $('.default input:radio').screwDefaultButtons({ 
        image: "url(../../external/images/radio.png)",
        width:   44,
        height:  42
    });
    ...
    $('.paymentPlan input:radio').screwDefaultButtons({ 
        image: "url(../../external/images/subscriptionRadio.png)",
        width:   44,
        height:  42
    });
...
var caretforkeyboardpos = 0;

$(document).ready(function() {
    $('.icon-keyboard').click(function() {
        $('#keyboard').slideToggle(200);
    });
    $('#keyboard a').click(function() {
        if ($(this).is(".icon-uppercase")){
            $("#keyboard a").each(function(i) {
                if (keyboardupper)
                    $(this).html(keyboardlowercase($(this).html()));
                else
                    $(this).html(keyboarduppercase($(this).html()));
            });
            ...
            if (elem.length > 0){
                ...
                // fix IE9
                if (elem.get(0).value == elem.attr("placeholder"))
                    elem.get(0).value = "";

                elem.insertAtCaret(value);
...
function keyboardlowercase(key) {
    if (key == 'SS')
        return 'ß';
    else
...
function keyboarduppercase(key) {
    if (key == 'ß')
        return 'SS';
    else
    ...
    insertAtCaret : function(myValue) {
        return this.each(function(i) {
            if (document.selection) {
                ...
                var t = this.value;
                this.value = t.substring(0, caretforkeyboardpos) + myValue + t.substring(caretforkeyboardpos);
                caretforkeyboardpos++;
...
//copy the Velocity array dictCodesShort to JavaScript:
var dictCodesShort = new Array();
var dictCodesAutocomplete = new Array();
var dictCodesUrl = new Array(); 
            dictCodesShort["english"] = "in Dictionary";
        dictCodesAutocomplete["english"] = "Dictionary"
        dictCodesUrl["english"] = "english";
                dictCodesShort["american_english"] = "in Dictionary";
        dictCodesAutocomplete["american_english"] = "Dictionary"
        dictCodesUrl["american_english"] = "american_english";
                dictCodesShort["english-thesaurus"] = "in Thesaurus";
        dictCodesAutocomplete["english-thesaurus"] = "Thesaurus"
        dictCodesUrl["english-thesaurus"] = "english-thesaurus";
                dictCodesShort["american_english-thesaurus"] = "in Thesaurus";
        dictCodesAutocomplete["american_english-thesaurus"] = "Thesaurus"
        dictCodesUrl["american_english-thesaurus"] = "american_english-thesaurus";
                dictCodesShort["english-french"] = "in English <i class=icon-arrow-right></i> French";
        dictCodesAutocomplete["english-french"] = "English <i class=icon-arrow-right></i> French"
        dictCodesUrl["english-french"] = "english-french";
                dictCodesShort["french-english"] = "in French <i class=icon-arrow-right></i> English";
        dictCodesAutocomplete["french-english"] = "French <i class=icon-arrow-right></i> English"
        dictCodesUrl["french-english"] = "french-english";
                dictCodesShort["english-german"] = "in English <i class=icon-arrow-right></i> German";
        dictCodesAutocomplete["english-german"] = "English <i class=icon-arrow-right></i> German"
        dictCodesUrl["english-german"] = "english-german";
                dictCodesShort["german-english"] = "in German <i class=icon-arrow-right></i> English";
        dictCodesAutocomplete["german-english"] = "German <i class=icon-arrow-right></i> English"
        dictCodesUrl["german-english"] = "german-english";
                dictCodesShort["english-italian"] = "in English <i class=icon-arrow-right></i> Italian";
        dictCodesAutocomplete["english-italian"] = "English <i class=icon-arrow-right></i> Italian"
        dictCodesUrl["english-italian"] = "english-italian";
                dictCodesShort["italian-english"] = "in Italian <i class=icon-arrow-right></i> English";
        dictCodesAutocomplete["italian-english"] = "Italian <i class=icon-arrow-right></i> English"
        dictCodesUrl["italian-english"] = "italian-english";
                dictCodesShort["english-spanish"] = "in English <i class=icon-arrow-right></i> Spanish";
        dictCodesAutocomplete["english-spanish"] = "English <i class=icon-arrow-right></i> Spanish"
        dictCodesUrl["english-spanish"] = "english-spanish";
                dictCodesShort["spanish-english"] = "in Spanish <i class=icon-arrow-right></i> English";
        dictCodesAutocomplete["spanish-english"] = "Spanish <i class=icon-arrow-right></i> English"
        dictCodesUrl["spanish-english"] = "spanish-english";
                dictCodesShort["harts_rules"] = "in New Hart's Rules";
        dictCodesAutocomplete["harts_rules"] = "New Hart's Rules"
        dictCodesUrl["harts_rules"] = "harts_rules";
                    dictCodesShort["garner_dict_legal_usage"] = "in Garner's Legal";
        dictCodesAutocomplete["garner_dict_legal_usage"] = "Garner's Legal"
        dictCodesUrl["garner_dict_legal_usage"] = "garner_dict_legal_usage";
                    dictCodesShort["ox_dict_writers_editors"] = "in Writers & Editors";
        dictCodesAutocomplete["ox_dict_writers_editors"] = "Writers & Editors"
        dictCodesUrl["ox_dict_writers_editors"] = "ox_dict_writers_editors";
                dictCodesShort["pocket_fowlers_modern_eng_usage"] = "in Fowler's Usage";
        dictCodesAutocomplete["pocket_fowlers_modern_eng_usage"] = "Fowler's Usage"
        dictCodesUrl["pocket_fowlers_modern_eng_usage"] = "pocket_fowlers_modern_eng_usage";
                dictCodesShort["words"] = "in Grammar & usage";
        dictCodesAutocomplete["words"] = "Grammar & usage"
        dictCodesUrl["words"] = "words";
    
var bilingualDictionaries = new Array();
    bilingualDictionaries.push("english-french");
    bilingualDictionaries.push("french-english");
    bilingualDictionaries.push("english-german");
    bilingualDictionaries.push("german-english");
    bilingualDictionaries.push("english-italian");
    bilingualDictionaries.push("italian-english");
    bilingualDictionaries.push("english-spanish");
    bilingualDictionaries.push("spanish-english");

function initAutocomplete() {
    var dictCode = $('#dictionaryselector').val();
    if (dictCode != undefined) {
        var url = "http://www.oxforddictionaries.com/search/" + dictCode + "/";
        $("#quickLinks").attr("action", url);
    }
    $("input#q").autoCompleter({
        url: "http://www.oxforddictionaries.com/autocomplete/" + dictCode + "/?multi=1", 
        minChars: 2,
        ...
        confirmSuggestionCallback: function (row) {
            ...
            url = "http://www.oxforddictionaries.com/search/" + autocompdict + "/";
            $("#quickLinks").attr("action", url);
            $('#quickLinks').submit();
        },
        queryCallback: function(callback) {
            var dictCode = $("#dictionaryselector").val();
            var url = "http://www.oxforddictionaries.com/autocomplete/" + dictCode + "/?multi=1";
            ...
            $.getJSON(url, params, function(data) {
                ...
                $.each(data['results'], function(key, val) {
                    var dictCode = val.dictionaries[0];
                    ...
                    result["results"][dictCode].push(row);
                });
                callback(result);
            });
        },
        createResultRowCallback: function(x, y) {
            var suggestionCategory = $("<div class='suggestionCategory'><span>" + dictCodesAutocomplete[x] + "</span></div>");

            var suggestionList = $("<div class='suggestionList'></div>");
            var suggestionListUl = $("<ul></ul>");
            for (var result in y){
                suggestionListUl.append("<li><a class='suggestionResult' data-value='" + dictCodesUrl[x] + "'>" + y[result].searchText + "</a></li>");
            }
            suggestionList.append(suggestionListUl);

            var className = "suggestionRow";
            if (bilingualDictionaries.indexOf(x) >= 0)
                className += " translation";

            if(data_access.indexOf("DATA_" + x) == -1)
                className += " premium";

            var suggestionRow = $("<div class='" + className + "'></div>");
        ...
        footerLinkCallback: function(){
            _gaq.push(['_trackEvent', 'results_list', $("#q").val()]);
            $('#quickLinks *[name="direct"]').attr('disabled', 'disabled');
            $('#quickLinks').submit();
        }
    });
}
...
$(document).ready(function(){
    ...
    // autocomplete
    initAutocomplete();
    $("#dictionaryselector").change(function(){
        initAutocomplete();
        _gaq.push(['_trackEvent', 'dictionarySelector', $(this).val()]);
    });

    // language selector
    $("#footerVersion").change(function(){
        document.location.href=$("#footerVersion").val();
    });
    ...
    // popups
    $("*[data-popup-href]").click(function(){
        if (this.popup)
            this.popup.remove();

        var url = $(this).attr("data-popup-href");
        this.popup = $("<div class='reveal-modal'><iframe src='" + url + "' onload='resizeIframe(this);' scrolling='no'></iframe><a class='close-reveal-modal'>×</a></div>");
    ...
    $(".moreInformationExemples").click(function(){
        ...
        var otherPanels = link.nextAll(".entrySynList");
        var otherLinks = link.nextAll(".moreInformationSynonyms");
        moreInformationLinkClick(link, panel, otherPanels, otherLinks);
    });

    // synonyms links
    $(".moreInformationSynonyms").click(function(){
        var link = $(this);
        var panel = link.nextAll(".entrySynList");
        var otherPanels = link.prevAll(".sentence_dictionary");
        var otherLinks = link.prevAll(".moreInformationExemples");
        moreInformationLinkClick(link, panel, otherPanels, otherLinks);
    });
    
    // in other dictionaries block
    $(".linkOtherDictionaries").click(function(){
        var link = $(this);
        var panel = link.nextAll(".blockOtherDictionaries");
        var otherPanels = null;
        var otherLinks = null;
        moreInformationLinkClick(link, panel, otherPanels, otherLinks);
    });

    // custom select
    $('#dictionaryselector').show().customSelect({labelAttribute: 'data-label'});
    $('#footer select').show().customSelect();
    ...
    // placeholder workaround for IE9
    ...
    $(".quiz").each(function(){
        var id = $(this).attr("id");
        initQuiz(id, quizJSON);
    });

    // signup/tax countries
    if ($("#contactCountry, #country").length > 0){
        $("#contactCountry, #country").change(function(){
    ...
    // expandable menu
    $(".expandable a[href='" + document.location.href + "']").parents(".expandable").children("div").children(".toggle.icon-plus").click();

    // autofocus only for desktop if the user is on the top of the page
    if(window.matchMedia("(min-width: 762px)").matches && $(document).scrollTop() == 0)
        $("#q").focus();
});

function resizeIframe(obj) {
    if (obj.style.height != obj.contentWindow.document.body.offsetHeight + 'px')
        obj.style.height = obj.contentWindow.document.body.offsetHeight + 'px';
    if (obj.style.width != obj.contentWindow.document.body.offsetWidth + 'px')
        obj.style.width = obj.contentWindow.document.body.offsetWidth + 'px';
    setTimeout(function(){resizeIframe(obj);},500);
}

function validateSearchForm(searchForm) {
    var value = searchForm.q.value;
    if (!value.length) {
        return false;
    } else if ((value.indexOf('?') >= 0 && value.indexOf('?') < 1) || (value.indexOf('*') >= 0 && value.indexOf('*') < 1)) {
        alert("Wildcards are not supported at the start of each search terms");
        return false;
    }
    else if(value.indexOf(' ') !== -1){
        var words = value.split(" ");
        for(var i=0;i<words.length;i++){
            if( words[i].indexOf('?') == 0 || words[i].indexOf('*') >= 0 &&  words[i].indexOf('*') < 1 ){
                alert("Wildcards are not supported at the start of each search terms");
                return false;
            }
        }
    ...
    if( value.indexOf("?") !== -1 || value.indexOf("*") !== -1 ){
        $('input[name="direct"]').attr('disabled', 'disabled');
    }
    // log the searched keyword to COUNTER
    ntptSubmitTag(searchForm, "event_type=quick_search&keyword=" + value);
    return true;
}

/* social */

function citeUpdate(select) {
    var citeId = select.options[select.selectedIndex].value;
    $("#previewWrapper p").hide();
    $("#"+citeId).show();
}

function citeFormatValid(form, noFormatErrorMsg) {
    if(form.format.selectedIndex < 1) {
        alert(noFormatErrorMsg);
        return false;
    }
    return true;
}
...
function moreInformationLinkClick(link, panel, otherPanels, otherLinks){
    if (panel.is(":visible")){
